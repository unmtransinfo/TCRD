#! /usr/bin/env python
'''
  Python Interfate to IDG-KMC Target Central Repository Database

  Steve Mathias
  smathias@salud.unm.edu
  Time-stamp: <2017-03-16 12:03:36 smathias>
'''
...
class DBAdaptor:
  ...
  def ins_xref(self, init, commit=True):
    ...
    with closing(self._conn.cursor()) as curs:
      try:
        ...
        if e[1].startswith('Duplicate entry') and "key 'xref_idx5'" in e[1]:
          pass
        else:
          self._conn.rollback()
  ...
  def ins_goa(self, init, commit=True):
    ...
    vals = ['%s','%s']
    for optcol in ['go_term', 'evidence', 'goeco']:
      if optcol in init:
        cols.append(optcol)
        vals.append('%s')
        params.append(init[optcol])
  ...
  def ins_pubmed(self, init, commit=True):
    ...
    with closing(self._conn.cursor()) as curs:
      try:
        ...
        if e[1].startswith('Duplicate entry') and "key 'PRIMARY'" in e[1]:
          pass
        else:
          self._conn.rollback()
  ...
  def get_protein_dto(self, dtoid):
    curs = self._conn.cursor(mysql.cursors.DictCursor)
