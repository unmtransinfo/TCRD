#!/usr/bin/env python
# Time-stamp: <2019-08-27 14:41:56 smathias>
...
GAT_IMPORT_FILE = '../SQL/TCRD6-gene_attribute_types.sql'
GA_EXPORT_FILE = '../exports/TCRD5-gene_attribute.csv'
GA_IMPORT_FILE = '../SQL/TCRD6-gene_attributes.sql'
...
def cnv_gene_attributes(args, idmap, ifn, ofn):
  ...
  gaid = 1
  with open(ofn, 'w') as ofh:
    ofh.write("LOCK TABLES `gene_attribute` WRITE;\n")
    ofh.write("/*!40000 ALTER TABLE `gene_attribute` DISABLE KEYS */;\n")
    ofh.write("INSERT INTO `gene_attribute` VALUES ")
    with open(ifn, 'r') as ifh:
      csvreader = csv.reader(ifh)
      header = csvreader.next() # skip header line
      rct = 1
      for row in csvreader:
        ...
        if v5pid in idmap:
          ...
          skip_ct += 1
          continue
        ofh.write('("{}","{}","{}","{}","{}")'.format(gaid,v6pid,row[2],row[3],row[4]))
        gaid += 1
        wct += 1
        if rct < line_ct:
          ofh.write(',')
  ...
  print "Processed {} lines.".format(rct)
  print "  Wrote inserts for {} new gene_attribute rows to file {}".format(wct, ofn)
